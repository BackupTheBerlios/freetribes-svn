<?
// I am reading through this file partly to get the evaluation process reoriented
// toward the new tables but I am also commenting things that I think there may be
// a better way of doing. Where I see something, I will comment it with // ^^^
//
// File: previously engineering.php
...
// ^^^ Why are we getting all tribes? Do we actually need to do this?
//     Given that this any structures to be built should be entries in the
//     activities table, do we actually need to get the tribe id's from elsewhere?
...
// ^^^ With respect to the above comment, would it not be just as easy to
//     add a line to the query above like ...
//     ORDER BY tribeid
//     and then loop through doing each each tribe?

//     That said, this file is only supposed to be dealing with engineering
//     and the method above is clearly going to pull a vast amount of unecessary
//     info from the tables and that will inevitably increase the overhead on the
//     scheduler, something that on a highly populated server might result in the
//     scheduler breaking if the timeout limit is reached

//     It does occur to me, though, that if we need rapid access to the info for
//     each tribe, as I suspect is the reason that it is written this way, that you
//     would be saving time IF you executed the query at the top BEFORE processing
//     ALL skills and stored the info in an array so that you only needed to do that
//     bit once

//     You could then execute a query that got all skills listed in the activities table
//     and stepped through each one that is actually listed there and processed all
//     tribes using each skill in one go. Again, it would be faster to pull the whole
//     activities table and stick it in an array.
//     Not sure about memory storage limitations in PHP though

    while( !$act->EOF )
    {
        $act_do = $act->fields;

// ^^^ From this point my comments in /* */ are intended to outline a code structure
//     that will be applicable to ANY skill - or at least the majority of them
...
/*
    $materials = $db->Execute("SELECT * FROM $dbtables[gd_products]"
                                "WHERE ")
            $mhp = $db->Execute("SELECT * FROM $dbtables[structures] "
                               ."WHERE complete = 'Y' "
                               ."AND long_name = 'meetinghouse' "

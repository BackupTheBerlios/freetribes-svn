...
// File: primaryskill.php
 $pos = (strpos($_SERVER['PHP_SELF'], "/primaryskill.php"));
if ($pos !== false)
{
    die("You cannot access this page directly!");
}

$res = $db->Execute("SELECT pri_skill_att,tribeid,clanid FROM $dbtables[tribes] WHERE pri_skill_att != ''");
db_op_result($res,__LINE__,__FILE__);
while( !$res->EOF )
{
    $tribe = $res->fields;
    ////////////////////////////////Get the info needed//////////////////////////////////
    $cur_pri = $db->Execute("SELECT abbr,level FROM $dbtables[skills] WHERE abbr = '$tribe[pri_skill_att]' AND tribeid = '$tribe[tribeid]'");
     db_op_result($cur_pri,__LINE__,__FILE__);
    $current_pri = $cur_pri->fields;

    $sinfo = $db->Execute("SELECT abbr,morale,long_name FROM $dbtables[skill_table] WHERE abbr = '$tribe[pri_skill_att]'");
      db_op_result($sinfo,__LINE__,__FILE__);
    $skinfo = $sinfo->fields;

    $lit = $db->Execute("SELECT level FROM $dbtables[skills] WHERE abbr = 'lit' AND tribeid = '$tribe[tribeid]'");
     db_op_result($lit,__LINE__,__FILE__);
    $literacy = $lit->fields;

    $resk = $db->Execute("SELECT level FROM $dbtables[skills] WHERE abbr = 'res' AND tribeid = '$tribe[tribeid]'");
       db_op_result($resk,__LINE__,__FILE__);
    $research = $resk->fields;

    // Queries added to get religious skill bonuses and penalties

    $religion = $db->Execute("SELECT arch_skill1,arch_skill2,pros_skill,arch_pen1,arch_pen2,arch_pen2_amount,arch_pen1_amount,pros_skill_amount,arch_skill1_amount,arch_skill2_amount FROM $dbtables[religions] WHERE clanid='$tribe[clanid]'");
    ...
    $totalchance = 0;
    ///////////////////////////////Set some variables//////////////////////////////////////
    $skillroll = abs(floor(mt_rand(1,100)));    ///bluesman addition
    ...
    {
        $current_pri['level'] += 1;
        $query = $db->Execute("UPDATE $dbtables[skills] SET level = '$current_pri[level]' WHERE tribeid = '$tribe[tribeid]' AND abbr = '$skinfo[abbr]'");
        db_op_result($query,__LINE__,__FILE__);
        $query = $db->Execute("UPDATE $dbtables[tribes] SET pri_skill_att = '' WHERE tribeid = '$tribe[tribeid]'");
        db_op_result($query,__LINE__,__FILE__);
        ...
        {
            $query = $db->Execute("UPDATE $dbtables[skills] SET level = '0' WHERE tribeid = '$tribe[tribeid]' AND abbr = 'res'");
              db_op_result($query,__LINE__,__FILE__);
        }

...
------------------------------------------------------------------------------------- */
    ...
    $tribes = $db->Execute("SELECT * FROM ".$dbtables['tribes']." WHERE tribeid = '$tribe_id' ");
    ...
    while(!$tribes->EOF)
    {
        ...
        $skills = $db->Execute("SELECT * FROM ".$dbtables['skills']." WHERE tribeid='".$tribe['tribeid']."' ORDER BY abbr ");
        db_op_result($skills,__LINE__,__FILE__);
        while (!$skills->EOF)
        {
            array_push($skill,$skills->fields);
            $skills->MoveNext();
        }
        // get a list of the activities that the tribe has planned

        $activities = $db->Execute("SELECT * FROM ".$dbtables['activities']." WHERE tribeid = '".$tribe['tribeid']."'");
        db_op_result($activities,__LINE__,__FILE__);
        while (!$activities->EOF)
        {
            ...
            $requirements = $db->Execute("SELECT * FROM ".$dbtables['gd_rq']." WHERE res_type='Product' AND res_key='".$activity['product']."' ORDER BY res_key, method, id");
             db_op_result($requirements,__LINE__,__FILE__);
            ...
            else
            {
                ...
                while (!$requirements->EOF)
                {
                    ...
                    while ($entry['method']==$method)
                    {
                        ...
                        $sql = "SELECT * FROM ".$dbtables[$entry['cl_table']]." WHERE ".$entry['cl_tribeidf']."='$source' ";

...
$menu = 27;
    ...
    if( $menu = 27 )
    {
        $iron = $db->Execute("SELECT * FROM $dbtables[hexes] "
                            ."WHERE res_type = 'Iron'");
        while( !$iron->EOF )
        {
            ...
            if( $basechance < 50 )
            {
                ...
                elseif( $typechance < 20 )
                {
                    $oretype = 'silver';
                }
                elseif( $typechance < 45 )
                {
                    $oretype = 'coal';
                }
                elseif( $typechance < 65 )
                {
                    $oretype = 'copper';
                }
                ...
                $db->Execute("UPDATE $dbtables[hexes] "
                            ."SET res_type = '$oretype' "
                            ."WHERE hex_id = '$ironhex[hex_id]' "
                            ."AND res_type = 'iron'");
                echo "Changing hex $ironhex[hex_id] from iron to $oretype.<BR>"; 
            }
            $iron->MoveNext();
        }
        die();
    ...
    if( $menu == 18 )
    {
       $db->Execute("DELETE FROM $dbtables[mapping]");
      $i = 1;
       while($i < 37501){
       $debugger = $db->Execute("INSERT INTO $dbtables[mapping] VALUES('','N','N','N','N')");
       if( !$debugger )
       {
           echo $db->ErrorMsg() . "<BR>";
       }
